...
W = np.load("training_data/weights.npy")
B = np.load("training_data/bias.npy")


def softmax(z):
    #z -= np.max(z)
    #z = np.exp(z).T / np.sum(np.exp(z))
    z = 1/(1+np.exp(-z))
    return z.T
...
while(1):
    ...
    if cv2.contourArea(contour) > 10000 and frames > 10:
        ...
        X = img2Save.reshape((1, img2Save.shape[0]*img2Save.shape[1]))

        predicted = softmax(np.dot(W.T,X.T)+B)
        predicted_class = np.where(predicted == np.max(predicted))[1][0]

        #print(str(predicted_class))
        cv2.putText(frame,'Predicted: '+str(predicted_class),(30,400), font, 1,(0,255,0),2,cv2.LINE_AA)
